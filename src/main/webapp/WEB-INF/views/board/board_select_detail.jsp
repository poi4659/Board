...
<script type="text/javascript">
	...
	function fn_fileDown(fileNum) {
		alert("파일 번호: " + fileNum); // fileNum 값 확인

		var formObj = $("form[name='readForm']");
		$("#FILENUM").attr("value", fileNum);

		/* 폼의 **action**을 ./BoardFileDown으로 설정 */
		formObj.attr("action", "./BoardFileDown");

		/* 폼을 전송하여 파일 다운로드 요청을 보냄 */
		formObj.submit();
	}
</script>
<body>
				...
				<div class="col-md-12">
					<div class="card">
						...
						<div class="card-body">
							...
							<!-- 첨부파일 -->
							<!-- 폼 제출을 위한 코드 추가 -->
							<!-- 파일 다운로드 시 fileNum 값을 숨겨진 입력 필드에 설정하여 제출 -->
							<form name="readForm" role="form" action="./BoardFileDown" method="POST">
								<input type="hidden" id="FILENUM" name="FILENUM" value="">
							</form>

							<%-- Map이기 때문에 ['key']방식으로 수정 --%>
							<%-- fileList: 첨부파일 목록이 담긴 리스트 --%>
							<c:if test="${not empty fileList}">
								<span>파일 목록</span>
								<c:forEach var="file" items="${fileList}">
									<%-- 파일 이름을 클릭하면 파일 다운로드가 시작되도록 fn_fileDown() 함수가 실행 --%>
									<a href="#" onclick="fn_fileDown('${file.FILENUM}'); return false;">
										<p>${file['ORGFILENAME']}
									</a>(${file['MPFILESIZE']}kb)</p>
								</c:forEach>
							</c:if>
